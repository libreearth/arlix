defmodule Arlix.Utils do
  ...
  ## Examples
      ...
      iex> get_in(list, [Arlix.Utils.access_by_id(2), Access.key(:name)])
      "Amy"

      iex> list = [%{id: 1, name: "Jake"}, %{id: 2, name: "Amy"}]
      iex> put_in(list, [Arlix.Utils.access_by_id(2), Access.key(:name)], "Amy Santiago")
      [%{id: 1, name: "Jake"}, %{id: 2, name: "Amy Santiago"}]

  An error is raised if the accessed structure is not a list:

      iex> get_in(%{}, [Arlix.Utils.access_by_id(1)])
      ** (RuntimeError) Arlix.Utils.access_by_id/1 expected a list, got: %{}
  """
  ...
  def access_by_id(id) do
    fn
      ...
      _op, data, _next ->
        raise "Arlix.Utils.access_by_id/1 expected a list, got: #{inspect(data)}"
    end
  end
