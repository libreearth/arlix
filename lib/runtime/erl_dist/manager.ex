defmodule Arlix.Runtime.ErlDist.Manager do
  @moduledoc false

  # The primary Arlix process started on a remote node.
  #
  ...
  alias Arlix.Evaluator
  alias Arlix.Runtime.ErlDist
  ...
  ## Options
    ...
    * `:cleanup_on_termination` - configures whether
      ...
      it altered and unload Arlix-specific modules
  ...
  See `Arlix.Runtime` for more details.
  """
  ...
  This results in all Arlix-related modules being unloaded from this node.
  """
  ...
  def init(opts) do
    ...
    Logger.add_backend(Arlix.Runtime.ErlDist.LoggerGLBackend)
  ...
  def terminate(_reason, state) do
    if state.cleanup_on_termination do
      ...
      Logger.remove_backend(Arlix.Runtime.ErlDist.LoggerGLBackend)
  ...
  def handle_cast(
    ...
    if evaluator = Map.get(state.evaluators, container_ref) do
      ...
      Task.Supervisor.start_child(state.completion_supervisor, fn ->
        ...
        items = Arlix.Completion.get_completion_items(hint, binding, env)
