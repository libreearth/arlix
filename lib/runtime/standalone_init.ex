defmodule Arlix.Runtime.StandaloneInit do
  @moduledoc false
  ...
  def elixir_flags(node_name) do
    [
      if(Arlix.Utils.longname(), do: "--name", else: "--sname"),
      to_string(node_name),
      "--erl",
    ...
    loop = fn loop ->
      receive do
        {:node_started, init_ref, ^child_node, primary_pid} ->
          ...
          :ok = Arlix.Runtime.ErlDist.initialize(child_node)
  ...
  receive do {:node_initialized, ^init_ref} ->\
    manager_ref = Process.monitor(Arlix.Runtime.ErlDist.Manager);\
